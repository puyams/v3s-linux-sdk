#!/bin/bash
#
# ./pack_zero_img.sh boot rootfs diy
#
...
if [ -n "$_BOOT" -a -n "$_ROOTFS" ]
	then if ([ -s $_BOOT/boot.scr -a -s $_BOOT/u-boot-sunxi-with-spl.bin -a -s $_BOOT/uImage ])
		then
		      echo_log "Checking the files ok!"
		else  echo_error "Can not find boot.scr, uImage, uboot or the modules file!" && exit
	     fi
